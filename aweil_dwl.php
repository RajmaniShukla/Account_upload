...
$tab .= "<table ><tr><td style=\"text-align:center;font-size:24px;\">Yearly Documents</td></tr>";
	 ...
	 while ($r_rec)
	 {
	   ...
	   $a_tag = '<a style="cursor:pointer;" onclick="window.open(\'dwl_all.php?nam='.$nam.'\')">';
	   $tab.= "<tr><td width=100%><div style='float:left;'>
				<span id='".$r_rec['cd']."'>".$a_tag."<font color='red' size='5'>&#xFF0B</font></span>
				</div><div style='float:left;padding-left:10px;font-size:26px;'>".$rem."</div>
				</a></td>
				</tr>";
	 ...
	 $tab .= "<table valign='top'>
				<tr>
					<td style=\"text-align:center;font-size:24px;\">Monthly Documents (12 Months)</td>
					</tr>";
	 $tab .="<tr><td width=100%>";
	 echo $tab;
	 //
    ...
    echo $tab;
	 ...
	 while ($r_rec)
	 {
	   ...
	   $prsn = chk_files($nam);
	    if ($prsn == "Y")	 
	   $tab = '<li>
				<a style="cursor:pointer;" onclick="window.open(\'dwl_all.php?nam='.$nam.'\')">'.$r_rec['des'].'</a>
				</li>';
	   else
	 ...
	 while ($r_rec)
	 {
	    $nam_des = $fin_yr."_MON-RECO_".$r_rec['cd'];
	    $prsn = chk_files($nam_des);
	    if ($prsn == "Y")
		$dwl_img = '<a style="cursor:pointer;" onclick="window.open(\'dwl_all.php?nam='.$nam_des.'\')">'.$r_rec['des'].'</a>';
		else
		$dwl_img = $r_rec['des'];
	    $tab= '<li><span class="caret"></span>'.$dwl_img;//.'</span>';
		$tab.= '<ul class="nested">';
		echo $tab;
	 ...
	 if ($rr_rec['cnt'] > 0)
	 {
		...
		$rrs_rec = mysqli_query ($con,$ssql_stmt);
	 	...
	 	{    
	  	     ...
	  	     $prsn = chk_files($nam);
	         if ($prsn == "Y")
			   $tab = '<li><a style="cursor:pointer;" onclick="window.open(\'dwl_all.php?nam='.$nam.'\')">'.$rr_rec['des'].'</a></li>';
			   else
	 ...
	 if ($rr_rec['cnt'] > 0)
	{
	 ...
	 while ($rrr_rec)
	 {
	   ...
	   $nam .= $rrr_rec['cd']."_";
	   $len = strlen($nam);
	   $nam_des = substr($nam,0,$len-1);
	   $prsn = chk_files($nam_des);
	   if ($prsn == "Y")
	   	$dwl_img = '<a style="cursor:pointer;" onclick="window.open(\'dwl_all.php?nam='.$nam_des.'\')">'.$rrr_rec['des'].'</a>';
	   else
	    $dwl_img = $rrr_rec['des'];
	    $tab= '<li><span class="caret"></span>'.$dwl_img;
	   //echo "join:",$nam," ",$sl,"<br>";
	   //$tab= '<li><span class="caret"></span>'.$rrr_rec[des];//.'</span>';
		$tab.= '<ul class="nested">';
		echo $tab;
