...
*/
  ...
  var requrl = "list_downloadable_files.php?nam="+val;
...
<form id='upl_dwl'  name = 'upl_dwl' method='POST' action ="aweil_dwl_selective_view.php">
...
$tab .= "<table ><tr><td style=\"text-align:center;font-size:24px;\">Yearly Documents</td></tr>";
	 ...
	 while ($r_rec)
	 {
	   ...
	   $rem = $r_rec['des'];
	   $un = 'unfold';//
	     $a_tag = "<a style='cursor:pointer;' onclick='list_files(\"".$nam."\",\"".$r_rec['cd']."\",\"".$un."\")'>";
	   $tab.= "<tr><td width=100%><div style='float:left;'><span id='".$r_rec['cd']."'>".$a_tag."<font color='red' size='5'>&#xFF0B</font></a></span></div><div style='float:left;padding-left:10px;font-size:26px;'>".$rem."</div><br><span id='span-".$r_rec['cd']."'></span></td></tr>";
    ...
    echo $tab;
	 ...
	 while ($r_rec)
	 {
	   ...
	   $prsn = chk_files($nam);
	    if ($prsn == "Y")	 
	   $tab = '<li><a style="cursor:pointer;" onclick="window.open(\'aweil_download_files.php?nam='.$nam.'\')">'.$r_rec['des'].'</a></li>';
	   else
	 ...
	 while ($r_rec)
	 {
	    ...
	    /*
        if ($prsn == "Y")
		$dwl_img = '<a style="cursor:pointer;" onclick="window.open(\'aweil_download_files.php?nam='.$nam_des.'\')">'.$r_rec['des'].'</a>';
		else
		$dwl_img = $r_rec[des];
*/
		$dwl_img = $r_rec['des'];
	    $tab= '<li><span class="caret">'.$dwl_img.'</span>';
		$tab.= '<ul class="nested">';
		echo $tab;
	 ...
	 if ($rr_rec['cnt'] > 0)
	 {
		...
		$rrs_rec = mysqli_query ($con,$ssql_stmt);
	 	...
	 	{    
	  	     ...
	  	     $prsn = chk_files($nam);
	         if ($prsn == "Y")
			   $tab = '<li><a style="cursor:pointer;" onclick="window.open(\'aweil_download_files.php?nam='.$nam.'\')">'.$rr_rec['des'].'</a></li>';
			   else
	 ...
	 if ($rr_rec['cnt'] > 0)
	{
	 ...
	 $rrrs_rec = mysqli_query ($sssql_stmt);
